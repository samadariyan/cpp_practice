...
export void tinker_with_unique_pointers() {
    ...
    */
    ...
    //----------------------------------------------------------------------------------
    // Using unique_ptr: managed memory is owned by a single pointer at any
    // moment.
    /*
    {
        int* p_int3 = new int(30);
        std::unique_ptr<int> up_int4{p_int3}; // Can also manage a previously
    allocated
                                                // space managed by a raw
    pointer. You shouldn't
                                                // try to use the raw pointer
    from this point on std::unique_ptr<int> up_int5 {new int(50)};
    std::unique_ptr<int> up_int6{nullptr};// Can also initialize with nullptr
                        // and give it memory to manage later, we'll see how to
                        // do that with std::move later in the lecture. Just
    know
                        // that you can initialize a unique ptr with nullptr for
    now.
    ...
    */
    ...
    std::unique_ptr<int> up_int9 = up_int8; // Error.This also does some kind of
    copy
                        // More on this when we've learnt about operator
    overloading
    //std::unique_ptr<int> up_int10{up_int8}; // Error :  Copy constructor
    deleted
    */
    ...
    {
        std::unique_ptr<int> up_int12 = std::move(up_int11); // Now up_int12
    manages int11
                                                    // and up_int11 points to
    nothing(nullptr) fmt::println( "up_int12 pointing at address :{}" ,
    fmt::ptr(up_int12.get() ));

        fmt::println( "up_int11 is now nullptr : {}" ,fmt::ptr( up_int11.get()
    )); if(up_int11){ fmt::println( "Pointer11 pointing to something valid" );
        }else{
    ...
    */
    ...
    // valid
    if (up_int13) {
        fmt::println("up_int13 points somewhere valid : {}",
                     fmt::ptr(up_int13.get()));
    } else {
