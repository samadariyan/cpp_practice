...
std::size_t Canvas::get_total_canvas_count() { return s_canvas_count; }
...
Canvas& Canvas::operator=(Canvas&& other) noexcept {
    ...
    swap(other);
    // doint this does not guarantee that the contents of this is immediately
    // cleaned up. Instead, the contents of this escapes through other to the
    // caller of the move assignment operator and thus might stay alive longer
    // than expected.
    return *this;
}
...
void swap(Canvas& first, Canvas& second) noexcept { first.swap(second); }
