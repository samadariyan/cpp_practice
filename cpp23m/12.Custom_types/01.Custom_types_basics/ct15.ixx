...
export bool write_pod(const std::filesystem::path& filename,
                      const PODType& pod) {
...
export bool
write_pod_vector(const std::filesystem::path& file_path,
    ...
    for (const auto& pod : pod_vector) {
        out_file.write(reinterpret_cast<const char*>(&pod),
                       sizeof(pod_types::PODType));
    }
...
export bool read_pod_vector(const std::filesystem::path& file_path,
    ...
    while (in_file.read(reinterpret_cast<char*>(&read_pod),
                        sizeof(pod_types::PODType))) {
...
export class Person {
  public:
    ...
    void deserialize(std::ifstream& ifs) {
        ...
        // Read the name and age from the file and store in the object's
...
export void write_persons_to_file(const std::vector<Person>& persons,
    ...
    fmt::println("Serialized {} person(s) to file: {}", vector_size,
                 file_path.string());
}

// Function to read a vector of person objects from a file
export std::vector<Person>
read_persons_from_file(const std::filesystem::path& file_path) {
    ...
    fmt::println("Deserialized {} person(s) from file: {}", vector_size,
                 file_path.string());
    return persons;
}
