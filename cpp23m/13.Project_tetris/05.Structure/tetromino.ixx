...
export class Tetromino {
  public:
    ...
    Tetromino(Type type)
        : current_type(type), current_rotation(0), pos_x(3), pos_y(0),
          last_pos_x(3), last_pos_y(0), last_rotation(0) {
    ...
    // When you rotate a 4×4 matrix clockwise, every element at position (i, j)
    // moves to (j, 3 - i)
    void rotate() {
  ...
  private:
    void initialize_shape(Type type) {
        ...
        case I:
            ...
            break;
        case O:
            current_shape[1][1] = current_shape[1][2] = current_shape[2][1] =
  ...
  private:
    ...
    int last_rotation;
    int pos_x; // x position on the board with a size of (width = 10, height =
               // 20) in our implementation
    int pos_y; // y position on the board with a size of (width = 10, height =
               // 20) in our implementation
    int last_pos_x;
    int last_pos_y;
