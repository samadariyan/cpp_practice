...
void draw_next_piece(sf::RenderWindow& window, const Tetromino& next_piece) {
    ...
    preview_box.setSize(
        sf::Vector2f(6 * Board::BLOCK_SIZE, 6 * Board::BLOCK_SIZE));
    preview_box.setPosition((Board::WIDTH + 0.5f) * Board::BLOCK_SIZE,
                            Board::BLOCK_SIZE);
...
void draw_score(sf::RenderWindow& window, int score, sf::Font& font,
                bool game_over) {
    sf::Text score_text;
...
export void app() {
    sf::RenderWindow window(
        sf::VideoMode((Board::WIDTH + 7) * Board::BLOCK_SIZE,
    ...
    while (window.isOpen()) {
        ...
        if (!game_over) {
            // Input handling with delay. The delay ensures that keypresses do
            // not register too frequently when a player holds down a movement
            // key. Without it, pressing and holding a key would move the piece
            // too quickly, making the game unplayable.
            ...
            if (fall_clock.getElapsedTime().asSeconds() >= fall_delay) {
                ...
                if (board.is_collision(current_piece)) {
                    // Brind the piece back to its previous position, if it
                    // falls our of range
                    current_piece.undo_move();
...
void draw_next_piece(sf::RenderWindow& window, const Tetromino& next_piece){
    ...
    // Draw preview box
    sf::RectangleShape preview_box;
    preview_box.setSize(sf::Vector2f(6 * Board::BLOCK_SIZE, 6 *
Board::BLOCK_SIZE)); preview_box.setPosition((Board::WIDTH + 0.5f) *
Board::BLOCK_SIZE, Board::BLOCK_SIZE);
    ...
    sf::RectangleShape block(sf::Vector2f(Board::BLOCK_SIZE,
Board::BLOCK_SIZE)); block.setOutlineColor(sf::Color::White);
    block.setOutlineThickness(1.0f);
...
void draw_score(sf::RenderWindow& window, int score, sf::Font& font, bool
game_over) { sf::Text score_text; score_text.setFont(font);
    score_text.setCharacterSize(24);
    score_text.setFillColor(sf::Color::White);
    ...
    if (game_over) {
        score_text.setString("Game Over!\nScore: " + std::to_string(score) +
"\nPress R to restart"); } else { score_text.setString("Score: " +
std::to_string(score));
    }
...
export void app(){
    ...
    sf::RenderWindow window(sf::VideoMode((Board::WIDTH + 7) *
Board::BLOCK_SIZE, Board::HEIGHT * Board::BLOCK_SIZE), "Tetris");
    window.setFramerateLimit(60);
    Board board;
    BoardEntity entity(board);
    Tetromino current_piece(spawn_new_tetromino());
    ...
    sf::Clock fall_clock;
    const float move_delay = 0.1f;    // Delay between movements
    const float fall_delay = 0.5f;    // Time between falls: bring this down to
increase speed
    ...
    while ((window.isOpen()))
    {
        ...
        if(!game_over){
            // Input handling with delay. The delay ensures that keypresses do
not register
            // too frequently when a player holds down a movement key. Without
it, pressing
            // and holding a key would move the piece too quickly, making the
game unplayable.
...
our of range current_piece.undo_move();
