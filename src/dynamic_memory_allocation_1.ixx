...
export void pointers_gone_bad() {
    ...
    *p_number2 = 55; // Writing into junk address : BAD!
    fmt::println( "p_number2 : {}", fmt::ptr(p_number2)); // Reading from junk
    address. fmt::println( "Dereferencing bad memory" ); fmt::println(
    "*p_number2 : {}", *p_number2 );
    */
...
export void dangling_pointers() {
    ...
    */
    ...
    fmt::println( "p_number4(after deleting p_number3) -{} :  {}",
    fmt::ptr(p_number4) , *p_number4 );
    */
    ...
    */
    ...
    // releasing when necessary) , other pointers should only be able to
...
export void memory_leaks() {
    ...
    // Non deleted memory
    /*
    int *p_number {new int{67}}; // Points to some address, let's call that
    0x1111
    ...
    p_number = &number; // Now p_number points to 0x2222
                        // Memory at 0x1111 leaked. It was allocated to our
    program but we just lost access to it.
    */
...
export void dynamic_arrays() {
    ...
    int* p_students{
        new (std::nothrow) int[size]{}}; // All values initialized to 0
    ...
    if (p_scores) {
        fmt::println("size of scores (it's a regular pointer): {}",
                     sizeof(p_scores));
