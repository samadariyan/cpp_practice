    ...
    while (window.isOpen()) {
        ...
        if (clock.getElapsedTime().asSeconds() > move_interval) {
            for (size_t i{0}; i < shapes.size(); ++i) {
                ...
                if (position.x <= 0.f ||
                    position.x + circle_radius * 2 >= width) {
                    direction.x = -direction.x; // Reverse horizontal direction
                }
                if (position.y <= 0.f ||
                    position.y + circle_radius * 2 >= height) {
...
// Storing shapes on the heap and managing memory manually through new and
// delete: The worst way
namespace dm_2 {
export void app() {
    // Store data in pre-defined variable
    constexpr int width{600};
