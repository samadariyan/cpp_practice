...
export void non_const_pointer_to_non_const_data() {
    ...
    fmt::println(
        "Modifying the value pointed to p_number1 through the pointer: ");
    *p_number1 = 432;
...
export void pointer_to_const() {
    ...
    // Pointer to const
    // Pointer pointing to constant data : You can't modify the data through
    // pointer
    fmt::println("Pointer is modifiable, pointed to value is constant: ");
    int number3{632}; // Although you can omit the const on number3 here and it
                      // is still going to compile, it is advised to be as
                      // explicit as possible in your code and put the const in
                      // front. Make your intents CLEAR.
    ...
    fmt::println("Modifying the value pointed to by p_number3 through the "
...
export void where_const_applies() {
    ...
    // You'll get a compiler error :Invalid convertion from 'const type*' to
    // 'type*'.
...
export void const_data_const_pointer() {
    ...
    fmt::println("Changing value pointed to by p_number5 through the pointer "
                 "(Compile Error) ");
    //*p_number5 = 222; // Error : Trying to assign to read only location

    // Can't change where the pointer is pointing to: The pointer is now itself
    // constant
    fmt::println(
