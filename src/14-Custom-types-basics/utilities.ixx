...
export void ct1_demo() {
    /*
                . {:#010x} is a format specifier that will print the hex value
       with 10 digits and 0 padding. . #  indicates that the output should
       include the 0x prefix for hexadecimal numbers. . 010 indicates that the
       output should be 10 characters wide and zero-padded. . x indicates that
       the number should be printed in lowercase hexadecimal.
        */
...
export void ct9_demo() {
    ...
    */
    ...
    // 2. Passing by value
    // process_pixel(p1);  // Copy constructor is called to pass p1 to the
    // function

    // 3. Returning by value: This may not always call the copy constructor due
    // to RVO (Return Value Optimization)
    // ct9::Pixel p3 = create_pixel();  // Copy constructor may be called (or
    // elided)
...
export void ct10_demo() {
    ...
    // ct10::Pixel p2{0xFFFF0000};         // Uses color-only constructor, which
    // delegates to the three-argument constructor
    ct10::Pixel p3{0xFF00FF00, 100,
                   50}; // Directly uses the three-argument constructor
...
read_pod.id, read_pod.value); }else{ fmt::print("Failed to read PODType object
from file.\n");
    }
    */
    ...
    // std::filesystem::path file_path = R"(/path/to/your/input_file.bin)"; //
    // Linux
...
export void ct15_demo_non_pod() {
    ...
    std::vector<non_pod_types::Person> persons = {
        {"Alice", 30}, {"Bob", 25}, {"Charlie", 40}};
    std::filesystem::path file_path =
        R"(D:\sample_non_pod_vector.bin)"; // Windows
    // std::filesystem::path file_path = R"(/path/to/your/input_file.bin)"; //
    // Linux
    ...
    std::vector<non_pod_types::Person> loaded_persons =
        non_pod_types::read_persons_from_file(file_path);
