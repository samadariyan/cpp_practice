...
// Copy constructor
Canvas::Canvas(const Canvas& src) : Canvas(src.m_width, src.m_height) {
    for (std::size_t i = 0; i < m_width; ++i) {
...
{ other.m_width = 0; other.m_height = 0; other.m_pixels = nullptr;
}
*/
...
Canvas::Canvas(std::size_t width, std::size_t height)
    : m_width(width), m_height(height),
      m_pixels(width, std::vector<Pixel>(height)) {}
