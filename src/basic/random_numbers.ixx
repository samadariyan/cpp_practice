...
export void random_num_modern_cpp() {
    ...
    std::mt19937 marsenne{
        rd()}; // Initialize our mersenne twister with a random seed
...
export void random_num_example() {
    ...
    std::vector<std::string> predictions{
        ...
        "you sitting in the dark typing lots of lines of code on your dirty "
        "computer",
    ...
    // Use the marsenne twister to generate a random number and store that in a
    // variable named index.
    std::random_device rd; // Provides seeds with less predictability
    std::mt19937 marsenne{
        rd()}; // Initialize our mersenne twister with a random seed
    const int size(predictions.size() - 1); // The distribution expects constant
                                            // expressions in it's initializer.
    std::uniform_int_distribution distr{0,
                                        size}; // We want uniform distribution.
