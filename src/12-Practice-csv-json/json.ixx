...
export void write_json(const std::filesystem::path& file_path) {
    ...
    // For simplicity, I'll use a minimal JSON-like structure, as C++ doesn't
    // have built-in JSON handling. In real-world projects, you'd typically use
    // libraries like nlohmann/json. However, this example focuses on the
    // writing and reading process using the standard library.
    json_file << "{\n";
