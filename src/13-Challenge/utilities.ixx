...
// const fs::path contacts_file = R"(D:\contacts.txt)";  // This path is for
// Windows. Adapt if needed
const fs::path contacts_file =
    R"(/home/daniel/contacts.txt)"; // This path is for Linux. Adapt if needed
...
void ensure_file_exists() {
    if (!fs::exists(contacts_file)) {
        std::ofstream file(
            contacts_file); // Creates the file if it doesn't exist
    }
}
