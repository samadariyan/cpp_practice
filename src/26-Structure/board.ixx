...
export class Board {
  public:
    ...
    void update_tetromino(const Tetromino& tetromino) {
        ...
        for (int y = 0; y < Tetromino::GRID_SIZE; ++y) {
            for (int x = 0; x < Tetromino::GRID_SIZE; ++x) {
                if (shape[y][x]) {
                    // If the spot is turned on in the tetromino, turn it on in
                    // the board
                    int board_x = pos_x + x;
                    int board_y = pos_y + y;
                    if (board_x >= 0 && board_x < WIDTH && board_y >= 0 &&
                        board_y < HEIGHT) {
                        grid[board_y][board_x] =
                            type; // The types are mapped to colors in the
                                  // BoardEntity class draw method.
                    }
                }
            }
        }
    ...
    /*
        1. The loop starts from the bottom row (HEIGHT - 1) and moves upwards
       (--y).
        2. When a complete line is found (is_line_complete(y)), it is cleared
       (clear_line(y)).
        3. The lines_cleared counter is incremented.
        4. The ++y statement increments y to recheck the same row index in the
       next iteration because the rows above have shifted down.
        5. The loop control (--y) will then decrement y in the next iteration,
       effectively keeping y at the same value it was before the ++y statement.
    */
